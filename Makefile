...
check-coverage: ## Check the test coverage of changed files
		git fetch origin && git diff origin/main > ${PWD}/diff.txt && docker build -t strictlyphp/domantra . && docker run --user=$(shell id -u):$(shell id -g) --rm --name strictlyphp-domantra -v "${PWD}":/usr/src/myapp -w /usr/src/myapp strictlyphp/domantra ./build/check-coverage.sh
